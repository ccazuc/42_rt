...
/*   Updated: 2017/11/17 15:27:59 by ccazuc           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_light_position(t_light *light, char **datas, int *start)
{
	...
	if (!datas[*start + 1] || !datas[*start + 2] || !datas[*start + 3])
		ft_exit("Error, invalid file. Not enough parameters for light's position.", -1);
	if (!ft_str_isdigit(datas[*start + 1]) || !ft_str_isdigit(datas[*start + 2]) ||
	!ft_str_isdigit(datas[*start + 3]))
		ft_exit("Error, invalid file. Light's Position parameters are invalid.", -1);
...
void	parse_light_rotation(t_light *light, char **datas, int *start)
{
	...
	if (!datas[*start + 1] || !datas[*start + 2] || !datas[*start + 3])
		ft_exit("Error, invalid file. Not enough parameters for light's rotation.", -1);
	if (!ft_str_isdigit(datas[*start + 1]) || !ft_str_isdigit(datas[*start + 2]) ||
	!ft_str_isdigit(datas[*start + 3]))
		ft_exit("Error, invalid file. Light's Rotation parameters are invalid.", -1);
