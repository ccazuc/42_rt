...
unsigned int	get_pixel(t_env *env, char *img, int x, int y)
{
	...
	r = img[(y * env->window_width + x) * env->bpp / 8 + 2];
	g = img[(y * env->window_width + x) * env->bpp / 8 + 1];
	b = img[(y * env->window_width + x) * env->bpp / 8];
...
void			pixel_put_fsaa(t_env *env, int x, int y, unsigned int color)
{
	...
	g = (color & 0x00FF00) >> 8;
	b = (color & 0x0000FF);
	env->fsaa_img[y * env->window_width * env->bpp / 8 + env->bpp
		/ 8 * x] = b;
	env->fsaa_img[y * env->window_width * env->bpp / 8 + env->bpp
		/ 8 * x + 1] = g;
	env->fsaa_img[y * env->window_width * env->bpp / 8 + env->bpp
